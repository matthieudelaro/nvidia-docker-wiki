...
```
`nvidia-docker` only modifies the behavior of the `run`, `create` and `volume` docker commands. All the other commands are just pass-through to the docker CLI client. As a result, you can't use a GPU when building a Docker image.
